...
MANIFEST = json.dumps({'name':'Ledger — Trade Journal','short_name':'Ledger','start_url':'/','display':'standalone','background_color':'#07112a','theme_color':'#07112a','icons':[{'src':'/icon.svg','sizes':'any','type':'image/svg+xml','purpose':'any maskable'}]})
...
class Handler(BaseHTTPRequestHandler):
    ...
    def send_asset(self, body, content_type):
        b=body.encode();self.send_response(200);self.send_header('Content-Type',content_type);self.send_header('Content-Length',str(len(b)));self.send_header('Cache-Control','public, max-age=86400');self.send_header('X-Content-Type-Options','nosniff');self.end_headers();self.wfile.write(b)
    ...
    def do_GET(self):
        ...
        if path=='/manifest.webmanifest': return self.send_asset(MANIFEST, 'application/manifest+json; charset=utf-8')
        if path=='/icon.svg': return self.send_asset(ICON_SVG, 'image/svg+xml; charset=utf-8')
        if path=='/sw.js': return self.send_asset(SERVICE_WORKER, 'application/javascript; charset=utf-8')
        if path=='/api/me':
